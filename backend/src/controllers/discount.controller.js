...
export const getDiscountByPartyAndBrandAndCatalog = async (req, res) => {
    try {
        const SpecialDiscounts = await specialDiscountModel.findOne({
            party: req.params.party_id,
            brand: req.params.brand_id,
            catalog: req.params.catalog_id
        })
            .populate('party', 'name')
            .populate('brand', 'name')
            .populate('catalog', 'name');
        if (!SpecialDiscounts)
        {
            const NormalDiscounts = partyBrandSelectionModel.findOne({
                party: req.params.party_id,
                brand: req.params.brand_id,
            })
            return res.status(200).json({discount : NormalDiscounts.discount});
        }
        else{
            if(!SpecialDiscounts.status)
            {
                const NormalDiscounts = partyBrandSelectionModel.findOne({
                    party: req.params.party_id,
                    brand: req.params.brand_id,
        ...
        return res.status(200).json({discount : SpecialDiscounts.discount});
    } catch (error) {
        res.status(500).json({ message: "Error fetching discounts", error: error.message });
    }
};
