...
// Get selected brands by party
export const getSelectedBrandsByParty = async (req, res) => {
    try {
        const party = await Party.findById(req.params.party_id);
        if (!party) {
            return res.status(404).json({ message: "Party not found" });
        }
        res.status(200).json(party.selected_brands);
    } catch (error) {
        res.status(500).json({ message: "Error fetching selected brands", error: error.message });
    }
};
