...
export const getCatalogsByBrand = async (req, res) => {
    try {
      const { id } = req.params;
      const catalogs = await Catalog.find({ brand: id }); // Assuming brand is stored as an ObjectId
      res.status(200).json(catalogs);
    } catch (error) {
...
export const addCatalog = async (req, res) => {
    try {
        ...
        const newCatalog = new Catalog({ brand, name, price });
...
export const updateCatalog = async (req, res) => {
    try {
        ...
        if (existingCatalog) {
            return res.status(400).json({ message: "Catalog name already exists for this brand" });
        }
